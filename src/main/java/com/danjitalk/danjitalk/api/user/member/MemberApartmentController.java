...
public class MemberApartmentController {
    ...
    @PutMapping("/member-apartments/{memberApartmentId}")
    public ResponseEntity<ApiResponse<Void>> updateMemberApartment(
        @PathVariable Long memberApartmentId,
        @RequestBody UpdateMemberApartmentRequest request
    ) {
        memberApartmentService.updateMemberApartment(memberApartmentId, request);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body(ApiResponse.success(204, null, null));
    }
}
